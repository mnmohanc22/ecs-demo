...
# Load .env so all DB_* variables are available via os.getenv()

# Load .env from the same directory as this file.
# override=False ensures ECS task-definition / docker-compose env vars
# always take precedence over values in the .env file.
# If .env is absent (production image without the file), this is a no-op.
...
def build_db_config() -> dict:
    """Build DB config from environment variables loaded via .env file."""
    password = os.getenv("DB_PASSWORD", "")
    if not password:
        log.warning("DB_PASSWORD is not set — DB connections will fail")
    ...
    log.info("DB config built from environment variables", extra={
        "host":    config["host"],
        "port":    config["port"],
...
def get_db_connection():
    if not DB_CONFIG:
        raise RuntimeError("DB config not loaded — check DB_PASSWORD and DB_HOST in .env")
